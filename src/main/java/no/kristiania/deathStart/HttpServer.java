        ...
        while(true) {
            try (Socket socket = serverSocket.accept()) {
                ...
                if (!requestPath.equals("/echo")) {
                    File highFile = new File(fileLocation + requestPath);
                    socket.getOutputStream().write(("HTTP/1.1 200 OK\r\n" +
                            "Content-Length: " + highFile.length() + "\r\n" +
                            "Connection: close\r\n" +
                            "\r\n").getBytes());

                    new FileInputStream(highFile).transferTo(socket.getOutputStream());
                    ...
                    String body = queryParameters.getOrDefault("Body", "ALOHA!");
